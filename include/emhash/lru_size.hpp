// emhash LRU cache (size-based eviction)
...
template <typename First, typename Second> struct entry {
    ...
    entry(const First& key, const Second& value, uint32_t ibucket) : second(value), first(key) {
        bucket = ibucket;
        orderid = next_orderid();
    }

    entry(First&& key, Second&& value, uint32_t ibucket) : second(std::move(value)), first(std::move(key)) {
        bucket = ibucket;
        orderid = next_orderid();
    }

    template <typename K, typename V>
    entry(K&& key, V&& value, uint32_t ibucket) : second(std::forward<V>(value)), first(std::forward<K>(key)) {
        bucket = ibucket;
        orderid = next_orderid();
    }
    ...
    entry(entry&& pairT) : second(std::move(pairT.second)), first(std::move(pairT.first)) {
        bucket = pairT.bucket;
        orderid = pairT.orderid;
    }
    ...
    void rehash(uint32_t required_buckets) {
        ...
        if (_num_filled > EMHASH_REHASH_LOG) {
            ...
            snprintf(buff, sizeof(buff), "    _num_filled/load_factor/K.V/pack/next_orderid = %u/%.3f/%s.%s/%zd|%u",
                     _num_filled, load_factor(), typeid(KeyT).name(), typeid(ValueT).name(), sizeof(_pairs[0]),
