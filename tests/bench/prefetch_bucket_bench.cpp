...
template <typename Map> double bench_find_with_bucket_prefetch(Map& map, const std::vector<int>& keys) {
    size_t sum = 0;
    Timer t;
...
template <typename BaseMap> class PrefetchMap : public BaseMap {
public:
    using BaseMap::BaseMap;

    // Prefetch-enhanced find for batch lookups
    template <typename KeyIter> static size_t batch_find_sum(BaseMap& map, KeyIter begin, KeyIter end) {
        size_t sum = 0;
        auto it = begin;
