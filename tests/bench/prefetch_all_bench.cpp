...
template <typename Map> double bench_find(Map& map, const std::vector<int>& keys) {
    size_t sum = 0;
    Timer t;
    for (int k : keys) {
        auto it = map.find(k);
...
template <typename Map> void run_bench(const std::string& name, const std::vector<int>& keys) {
    Map map;
    ...
    printf("%-12s | insert: %5.0f ms | find(hit): %5.0f ms | find(miss): %5.0f ms | erase: %5.0f ms\n", name.c_str(),
           t_insert, t_find, t_miss, t_erase);
}
