...
int main() {
    ...
    printf("\n=== Test 3: Full crash sequence replay ===\n");
    {
        ...
        // Op 0: ERASE 8447
        em.erase(8447); ref.erase(8447);
        // Op 1: CLEAR
        em.clear(); ref.clear();
        // Op 2: INSERT (131072, 0)
        em.insert({131072, 0}); ref.insert({131072, 0});
        printf("After op 2: em.size=%u ref.size=%zu\n", em.size(), ref.size());
        // Op 3: FIND 394
        em.find(394); ref.find(394);
        // Op 4: RESERVE(1)
        em.reserve(1); ref.reserve(1);
        printf("After op 4 reserve(1): em.size=%u ref.size=%zu\n", em.size(), ref.size());
        // Op 5: INSERT (15663104, 13631725)
        em.insert({15663104, 13631725}); ref.insert({15663104, 13631725});
        printf("After op 5: em.size=%u ref.size=%zu\n", em.size(), ref.size());
        // Op 6: FIND 1744874668
        em.find(1744874668); ref.find(1744874668);
        // Op 7-9: ITERATE
        for (int i = 0; i < 3; i++) {
        ...
        em.count(-805245696); ref.count(-805245696);
