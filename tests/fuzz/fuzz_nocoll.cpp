...
// Portable trap: __builtin_trap for GCC/Clang, __debugbreak for MSVC, abort() fallback
#if defined(__GNUC__) || defined(__clang__)
#define EMH_TRAP() __builtin_trap()
#elif defined(_MSC_VER)
#define EMH_TRAP() __debugbreak()
#else
#include <cstdlib>
#define EMH_TRAP() std::abort()
#endif
    ...
    auto verify_all = [&]() {
        ...
        for (auto it = em.begin(); it != em.end(); ++it) {
            ...
            if (rit->second != it->second) {
                EMH_TRAP(); // VALUE MISMATCH
            }
            count++;
        }
        if (count != ref.size()) {
            EMH_TRAP(); // SIZE MISMATCH
        }
    };
    ...
    for (size_t i = 0; i < ops.size(); i++) {
        ...
        switch (op.code) {
            case OP_INSERT: {
                ...
                if (em_r.second != ref_r.second) EMH_TRAP();
                if (em_r.second && em_r.first->second != op.value) EMH_TRAP();
                break;
            }
            case OP_ERASE: {
                size_t em_r = em.erase(op.key);
                size_t ref_r = ref.erase(op.key);
                if (em_r != ref_r) EMH_TRAP();
                break;
            }
            case OP_FIND: {
                auto em_it = em.find(op.key);
                auto ref_it = ref.find(op.key);
                if ((em_it != em.end()) != (ref_it != ref.end())) EMH_TRAP();
                if (em_it != em.end() && ref_it != ref.end())
                    if (em_it->second != ref_it->second) EMH_TRAP();
                break;
            }
            case OP_ACCESS: {
                em[op.key] = op.value;
                ref[op.key] = op.value;
                if (em[op.key] != op.value) EMH_TRAP();
                break;
            }
            case OP_ITERATE: {
            ...
            case OP_COUNT: {
                if (em.count(op.key) != ref.count(op.key)) EMH_TRAP();
                break;
            }
            case OP_CONTAINS: {
                bool em_has = em.contains(op.key);
                bool ref_has = (ref.find(op.key) != ref.end());
                if (em_has != ref_has) EMH_TRAP();
                break;
            }
            case OP_EMPLACE: {
...
static void fuzz_extreme_hashset(const std::vector<Op>& ops) {
    ...
    auto verify_all = [&]() {
        for (const auto& k : ref) {
            if (!em.contains(k)) EMH_TRAP();
        }
        size_t count = 0;
        for (auto it = em.begin(); it != em.end(); ++it) {
            if (ref.count(*it) != 1) EMH_TRAP();
            count++;
        }
        if (count != ref.size()) EMH_TRAP();
    };
    ...
    for (const auto& op : ops) {
        switch (op.code) {
            ...
            case OP_FIND: {
                if ((em.find(op.key) != em.end()) != (ref.find(op.key) != ref.end())) EMH_TRAP();
                break;
            }
            case OP_ACCESS: {
                em.insert(op.key);
            ...
            case OP_CONTAINS: {
                if (em.contains(op.key) != (ref.count(op.key) > 0)) EMH_TRAP();
                break;
            }
            case OP_EMPLACE: {
...
static void fuzz_churn(const std::vector<Op>& ops) {
    ...
    for (const auto& kv : ref) {
        auto it = em.find(kv.first);
        if (it == em.end()) EMH_TRAP();
        if (it->second != kv.second) EMH_TRAP();
    }
    size_t count = 0;
    for (auto it = em.begin(); it != em.end(); ++it) {
        if (ref.count(it->first) != 1) EMH_TRAP();
        if (ref[it->first] != it->second) EMH_TRAP();
        count++;
    }
    if (count != ref.size()) EMH_TRAP();
}
