...
#if 0  // try_emplace disabled in emhash5 (TODO: fix tuple implementation)
	{
		ehmap<uint64_t, uint32_t> emi;
		emi.reserve(1000);
		uint64_t key = rand();
		emi.insert({key, 0}); emi.emplace(key, 1);
		auto it = emi.try_emplace(emi.end(), key, 0); assert(it == emi.find(key));
		it = emi.try_emplace(emi.end(), key + 1, 1); assert(it != emi.find(key));
...
void test_basic_operations() {
	...
	// 插入测试
	auto result1 = map.insert(emhash8::HashMap<int, std::string>::value_type(1, "one"));
	assert(result1.second);
	assert(result1.first->second == "one");

	map[2] = "two"; // ʹ��operator[]����
	assert(map.size() == 2);
	...
	assert(map.find(3) == map.end()); // ���Ҳ����ڵļ�
...
void test_edge_cases() {
	...
	single_map.insert(std::make_pair(0, 0));
	assert(single_map.size() == 1);
	assert(single_map.begin() != single_map.end());
	assert(std::next(single_map.begin()) == single_map.end());

	// �ظ��������
	auto dup_result = single_map.insert(std::make_pair(0, 100));
	assert(!dup_result.second);
	assert(dup_result.first->second == 0);  // ֵӦ���ֲ���
...
void test_rehash_reserve() {
	emhash8::HashMap<int, int> map;
	map.reserve(100);  // Ԥ����ռ�
	assert(map.bucket_count() >= 100);  // Ͱ����Ӧ����Ϊ100
	...
	map.rehash(200);  // ǿ���ع�ϣ��200��Ͱ
	assert(map.bucket_count() >= 200);
	assert(map.size() == 50);  // �ع�ϣ��Ԫ����������
...
struct CustomHash {
	size_t operator()(int x) const {
		return (size_t)x * 1234567;  // ���Զ����ϣ
	}
};

struct CustomEq {
	bool operator()(int a, int b) const {
		return a == b;  // �Զ�������ԱȽϣ���Ĭ����ͬ��
	}
};
...
void test_merge() {
	...
	map1.merge(map2);
	// map1Ӧ����1,2,3��map2Ӧ����2����Ϊmap1������2��
	assert(map1.size() == 3);
	assert(map1.at(2) == "two");  // ����ԭmap1��ֵ
	assert(map1.at(3) == "tres");
	assert(map2.size() == 1);  // map2�н�����δ�ϲ���2
...
void test_unique()
{
	...
	for (int loop = 0; loop < 10; loop++) {
		...
		auto nowus = getus();
		{
			...
			WyRand srng(maxn);
			for (int i = 1; i <= maxn; i++) {
				map1.emplace(srng(), true);
			}
			printf("insert %d use %d ms\n", maxn, int((getus() - nowus) / 1000));
		}

		if (1)
		{
			map1.clear();            nowus = getus();
			WyRand srng(maxn);
			for (int i = 1; i <= maxn; i++) {
				map1.emplace(srng(), true);
			}
			printf("operator[] %d use %d ms\n", maxn, int((getus() - nowus) / 1000));
		}
...
#if 1
		{
			...
			WyRand srng(maxn);
			for (int i = 1; i <= maxn; i++) {
				map1.insert_unique(srng(), true);
			}
			printf("insert_unique %d use %d ms\n", maxn, int((getus() - nowus) / 1000));
		}
#endif
