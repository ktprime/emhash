...
template <typename Map> int64_t measure_find_hit(Map& m, const std::vector<uint64_t>& keys, int iters) {
...
template <typename Map> int64_t measure_find_miss(Map& m, const std::vector<uint64_t>& miss_keys, int iters) {
    volatile size_t sink = 0;
    int64_t total = 0;
    for (int i = 0; i < iters; i++) {
        auto t0 = now_ns();
...
int main() {
    ...
    printf("  %-12s %12s %12s %8s %12s %12s %8s\n", "N", "opt_hit(ns)", "boost_hit(ns)", "ratio", "opt_miss(ns)",
           "boost_miss(ns)", "ratio");
    printf("  %s\n", std::string(78, '-').c_str());
    ...
    for (auto N : {10000, 100000, 1000000, 5000000}) {
        ...
        printf("  %-12d %12lld %12lld %7.2fx %12lld %12lld %7.2fx\n", N, (long long)em_hit, (long long)bm_hit,
               (double)em_hit / bm_hit, (long long)em_miss, (long long)bm_miss, (double)em_miss / bm_miss);
    }
