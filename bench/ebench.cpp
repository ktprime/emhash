...
struct StructValue {
    ...
    bool operator==(const StructValue& v) const { return v.lScore == lScore; }

    int64_t operator()() const { return lScore; }

    bool operator<(const StructValue& r) const { return lScore < r.lScore; }

    StructValue& operator*=(const int r) {
        lScore *= r;
        return *this;
    }
...
struct StuHasher {
    std::size_t operator()(const StructValue& v) const { return v.lScore * 11400714819323198485ull; }
};
...
static void hash_convert(const std::map<std::string, int64_t>& hash_score,
                         std::multimap<int64_t, std::string>& score_hash) {
    for (const auto& v : hash_score)
        score_hash.emplace(v.second, v.first);
}

static void add_hash_func_time(std::map<std::string, std::map<std::string, int64_t>>& func_hash_score,
...
static void dump_func(const std::string& func, const std::map<std::string, int64_t>& hash_rtime,
    ...
    for (const auto& v : rscore_hash) {
        ...
        printf("%4d        %-20s   %-2.1f %%\n", (int)(v.first / test_case), v.second.data(),
               ((v.first * 100.0f) / mins));
    }
    putchar('\n');
}
...
static void dump_all(std::map<std::string, std::map<std::string, int64_t>>& func_rtime,
    ...
    std::string pys = "import numpy as np\n"
    ...
    static std::vector<std::string> colors = {
        "cyan",   "magenta", "green", "red",  "blue",    "yellow",    "black",
        "orange", "brown",   "grey",  "pink", "#eeefff", "burlywood",
    };
...
template <class hash_type> static void insert_erase(const std::string& hash_name, const std::vector<keyType>& vList) {
    hash_type ht_hash;
    auto ts1 = getus();
    size_t sum(0);
    // small dataset
...
template <class hash_type>
static void insert_cache_size(const std::string& hash_name, const std::vector<keyType>& vList, const char* level,
                              const uint32_t cache_size, const uint32_t min_size) {
...
template <class hash_type> static void insert_erase_high(const std::string& hash_name, size_t vSize) {
#if TTKey < 2
    hash_type ht_hash;
    auto max_lf = 0.90f;
...
template <class hash_type> static void hash_clear(hash_type& ht_hash, const std::string& hash_name) {
...
template <class hash_type> static void copy_clear(hash_type& ht_hash, const std::string& hash_name) {
    size_t sum = 0;
    auto ts1 = getus();
...
template <class hash_type> static void benOneHash(const std::string& hash_name, const std::vector<keyType>& oList) {
...
static void reset_top3(std::map<std::string, int64_t>& top3,
                       const std::multimap<int64_t, std::string>& once_score_hash) {
...
static void printResult() {
    ...
    for (auto& v : top3)
        printf("%13s %4.1lf  %4.1lf %4d\n", v.first.data(), v.second / (double)(base1),
               (v.second / (base2 / 2) % 1000) / 2.0, (int)(v.second % (base2 / 2)));
    ...
    for (auto it = score_hash.rbegin(); it != score_hash.rend(); it++) {
        const auto& v = *it;
        printf("%13s  %4d     %3.1lf %%\n", v.second.data(), (int)(v.first / func_hash_score.size()),
               (maxs * 100.0 / v.first));
    }
...
static int benchHashMap(int n) {
...

    {
        ...
        printf("n = %d, keyType = %s, valueType = %s(%zd), loop_sum|loop_rand = %d|%d us, sum = %d\n", n, sKeyType,
               sValueType, sizeof(valueType), loop_vector_time, loop_rand, (int)sum);
    }
    ...
    constexpr uint64_t kv =
        sizeof(std::pair<keyType, valueType>);      // sizeof(keyType) + sizeof(valueType) + (eq ? 0 : 4);
    ...
    printf("\n %d ======== n = %d, load_factor = %.3lf(emh8/flat = %.2lf/%.2lf, smap/umap = %.2lf/%.2lf MB), data_type "
           "= %d ========\n",
...
int main(int argc, char* argv[]) {
    ...
    printf("./ebench maxn = %d c(0-1000) f(0-100) d[2-9 mpatseblku] a(0-3) b t(n %dkB - %dMB)\n", (int)maxn,
           minn * type_size >> 10, maxn * type_size >> 20);
