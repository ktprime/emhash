/**
 * Benchmark: emihmap3 (two-array layout) vs emihmap3_flat (flat group layout)
 * vs emihmap4_opt (Swiss table) vs boost::unordered_flat_map
 *
 ...
 *
 * Usage (WSL):
 *   bash run_emihmap3_flat_bench.sh
 */
...
int main() {
    ...
    for (int N : {10000, 100000, 1000000, 5000000}) {
        ...
        run_bench<emilib3_flat::HashMap<KI, VI>>("emihmap3_flat(flat group)", keys, miss_keys, (VI)1);
        run_bench<emilib4_opt::HashMap<KI, VI>>("emihmap4_opt(swiss)", keys, miss_keys, (VI)1);
        run_bench<boost::unordered_flat_map<KI, VI>>("boost::flat_map", keys, miss_keys, (VI)1);
    }
    ...
    for (int N : {10000, 100000, 1000000}) {
        ...
        printf("\n  [N = %d]\n", N);
        run_bench<emilib3::HashMap<std::string, VI>>("emihmap3(orig)", keys, miss_keys, (VI)1);
        run_bench<emilib3_flat::HashMap<std::string, VI>>("emihmap3_flat(flat group)", keys, miss_keys, (VI)1);
        run_bench<emilib4_opt::HashMap<std::string, VI>>("emihmap4_opt(swiss)", keys, miss_keys, (VI)1);
        run_bench<boost::unordered_flat_map<std::string, VI>>("boost::flat_map", keys, miss_keys, (VI)1);
    }
