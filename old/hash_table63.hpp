...
#include <cstring>
    ...
    #define EMH_BUCKET(p,n) p[n].nextbucket
    #define EMH_PKV(s,n)    s[n]
...
public:
    ...
    HashMap(const HashMap& other)
    {
        ...
        if (sizeof(PairT) <= 24) {
            ...
            for (uint32_t bucket = 0; bucket < _num_buckets; bucket++) {
                auto state = EMH_BUCKET(_pairs, bucket) = EMH_BUCKET(old_pairs, bucket);
                if (state != INACTIVE)
    ...
    int get_cache_info(int bucket, int next_bucket) const
    {
        ...
        if (diff < 127 * EMH_CACHE_LINE_SIZE)
...
//            printf("  %2d  %8d  %.2lf  %.2lf\n", i, steps[i], steps[i] * 100.0 / collision, sumc * 100.0 / collision);
    ...
    std::pair<iterator, bool> insert(const KeyT& key, const ValueT& value)
    {
        ...
        if (EMH_BUCKET(_pairs, bucket) != INACTIVE) {
            ...
            if (check_expand_need())
                bucket = find_unique_bucket(key, true);
    ...
    {
        check_expand_need();
        auto bucket = find_unique_bucket(key, true);
        EMH_NEW(key, value, bucket);
        _num_filled++;
        return bucket;
    }
    ...
    {
        auto bucket = erase_bucket(it->first);
        if (bucket == INACTIVE) {
            return ++it;
        }
    ...
    void rehash(uint32_t required_buckets)
    {
        ...
        for (uint32_t bucket = 0; bucket < collision; bucket++) {
            auto new_bucket = find_unique_bucket(EMH_KEY(old_pairs, bucket), false);
...
private:
    ...
    int erase_bucket(const KeyT& key)
    {
        const auto bucket = hash_bucket(key);
        auto next_bucket = EMH_BUCKET(_pairs, bucket);
        if (next_bucket == INACTIVE)
    ...
    int find_or_allocate(const KeyT& key)
    {
        ...
        if (next_bucket == INACTIVE || bucket_key == key)
            return bucket;
        else if (next_bucket == bucket && bucket == hash_bucket(bucket_key))
    ...
    int find_empty_bucket(int bucket_from) const
    {
        ...
        auto slot = 1;
        const auto bucket_address = (int)(reinterpret_cast<size_t>(&EMH_BUCKET(_pairs, bucket_from ++)) % EMH_CACHE_LINE_SIZE);
        const auto line_probe_length = (int)((EMH_CACHE_LINE_SIZE * 2 - bucket_address) / sizeof(PairT));
        ...
        for ( ; ; ++slot) {
            ...
            const auto cache_offset = reinterpret_cast<size_t>(&EMH_BUCKET(_pairs,bucket1)) % EMH_CACHE_LINE_SIZE;
            if (cache_offset + sizeof(PairT) < EMH_CACHE_LINE_SIZE) {
    ...
    int find_unique_bucket(const KeyT& key, bool check_main)
    {
        const auto bucket = hash_bucket(key);
        auto next_bucket = EMH_BUCKET(_pairs, bucket);
        if (next_bucket == INACTIVE)
    ...
    inline int hash_bucket(const UType key) const
    {
