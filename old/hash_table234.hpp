...
public:
	...
	{
		_states[bucket].flag_probe -= 1;
	}

	inline bool is_empty(size_t bucket) const
	{
		return (_states[bucket].flag_probe & 1) == State::INACTIVE;
	}

	inline void set_filled(size_t bucket)
	{
		_states[bucket].flag_probe |= State::FILLED;
	}
	...
	iterator erase(iterator it)
	{
		...
		if (_states[it._bucket].get_probe() == 1)
			_states[it._bucket].flag_probe = 0;
		else
...
private:
	...
	size_t find_or_allocate(const KeyT& key)
	{
		...
		if (cur_probe == 0 && is_empty(main_bucket)) {
