...
private:
	...
	size_t find_filled_bucket(const KeyT& key) const
	{
		...
		uint32_t offset = _states[hash_value & _mask].get_probe();
		while (offset > 0) {
	...
	{
		const auto hash_value = _hasher(key);
		const size_t main_bucket = hash_value & _mask;
		size_t hole = (size_t)-1, offset = 0;
		const size_t cur_probe = _states[main_bucket].get_probe();
	...
	size_t find_empty_bucket(const KeyT& key)
	{
		...
		for (size_t offset=_states[main_bucket].get_probe(); ; ++offset) {
