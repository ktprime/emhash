...
| File | Type | Target | Description |
|------|------|--------|-------------|
| `test_crud.cpp` | Unit | All versions | CRUD operations: insert, find, erase, update |
| `test_copy_move.cpp` | Unit | All versions | Copy/move constructors and assignment operators |
| `test_iterators.cpp` | Unit | All versions | Iterator traversal, increment, comparison |
| `test_reserve_clear.cpp` | Unit | All versions | Reserve, clear, shrink_to_fit, rehash |
| `test_edge_cases.cpp` | Unit | All versions | Boundary conditions: empty map, single element, max load |
| `test_special_keys.cpp` | Unit | All versions | Special key values: 0, -1, max, duplicate keys |
| `test_string_keys.cpp` | Unit | All versions | String key CRUD, copy/move, erase patterns |
| `test_full_api.cpp` | Unit | All versions | Full API coverage: at(), contains(), count(), equal_range(), merge(), erase_if() |
| `test_hashset.cpp` | Unit | All HashSet | HashSet-specific operations: insert, contains, erase, merge |
| `test_allocator.cpp` | Unit | All versions | Custom allocator support |
| `test_api_coverage.cpp` | Unit | All versions | API coverage validation |

### 2.2 memory/ — Memory and Sanitizer Tests

| File | Type | Target | Description |
|------|------|--------|-------------|
| `test_string_key_leak.cpp` | Memory | All versions | String key memory leak detection: balance, rehash, stress, move semantics |
| `test_sanitizer.cpp` | Memory | All versions | ASan/UBSan/MSan: clear(), erase+reinsert, bucket boundaries, high load factor |
| `test_lifecycle_audit.cpp` | Memory | All versions | Object lifecycle: construction, destruction, move, copy audit |

### 2.3 stress/ — Stress Tests

| File | Type | Target | Description |
|------|------|--------|-------------|
| `test_stress_all.cpp` | Stress | All versions | Insert/delete loop across all implementations |
| `test_bad_hash.cpp` | Stress | Bad hash | Test behavior under bad hash functions |
...
| File | Type | Target | Description |
|------|------|--------|-------------|
| `test_hash_attack.cpp` | Attack | All versions | Comprehensive hash attack: constant hash, small range hash, linear hash |
| `test_collision_hardening.cpp` | Attack | All versions | Collision hardening: EMH_SAFE_HASH, EMH_SAFE_PSL validation |
...
COV='-std=c++17 -O0 -g --coverage -fprofile-arcs -ftest-coverage -I../common -I../../include'
g++ $COV ../unit/test_crud.cpp                -o test_crud_cov
g++ $COV ../unit/test_edge_cases.cpp          -o test_edge_cov
g++ $COV ../unit/test_full_api.cpp            -o test_api_cov
g++ $COV ../unit/test_hashset.cpp             -o test_hashset_cov
g++ $COV ../memory/test_string_key_leak.cpp   -o test_leak_cov
g++ $COV ../memory/test_sanitizer.cpp         -o test_sanitizer_cov
g++ $COV ../memory/test_lifecycle_audit.cpp   -o test_lifecycle_cov
g++ $COV ../stress/test_stress_all.cpp        -o test_stress_cov
g++ $COV -DEMH_SAFE_PSL ../attack/test_hash_attack.cpp -o test_attack_cov

# Run all
for t in test_crud_cov test_edge_cov test_api_cov test_hashset_cov \
         test_leak_cov test_sanitizer_cov test_lifecycle_cov \
         test_stress_cov test_attack_cov; do
  ./$t
done
