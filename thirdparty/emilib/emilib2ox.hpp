...
//        const_iterator(const htype* hash_map, size_t bucket, bool) : _map(hash_map), _bucket(bucket) { init(); }
    ...
    void do_erase(size_t bucket)
    {
        ...
        if (EMH_UNLIKELY(_num_filled == 0)) {
            ...
            std::fill_n(_offset, _num_buckets / simd_bytes + 1, EMPTY_OFFSET);
        }
#endif
    }
...
//#define EMH_STATIS 2000'000
    ...
    void rehash(size_t num_elems) noexcept
    {
        ...
        const auto pairs_size = (num_buckets + 2) * sizeof(PairT);
        ...
        //fill offset to 0
        std::fill_n(_offset, num_buckets / simd_bytes + 1, EMPTY_OFFSET);
...
#endif
        return _offset[main_bucket / simd_bytes];
    }
...
//        if (off < _offset[main_bucket])
        ...
        _offset[main_bucket / simd_bytes] = off;
#endif
    }
