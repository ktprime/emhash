...
public:
    ...
    private:
        void goto_next_element()
        {
            ...
            do {
                _bucket++;
            } while ((_map->_states[_bucket].flag_probe & 1) == State::INACTIVE);
        }
    ...
    {
        _states[bucket].flag_probe |= State::INACTIVE;
    }
    ...
    {
        return (_states[bucket].flag_probe & 1) == State::FILLED;
    }
    ...
    void clear()
    {
        ...
        for (size_t bucket=0; bucket<_num_buckets; bucket++)
            _states[bucket].clear();
        //memset(_states, 0x00010001, sizeof(_states[0]) * _num_buckets);
    }
    ...
    void reserve(size_t num_elems)
    {
        ...
        auto new_states = (State*)malloc((1 + num_buckets) * sizeof(State) + 2 * sizeof(uint64_t));
        ...
        //memset(_states, 0x00010001, sizeof(_states[0]) * num_buckets);
        for (size_t bucket=0; bucket<_num_buckets; bucket++)
            _states[bucket].clear();
        ...
        for (size_t src_bucket=0; _num_filled < old_num_filled; src_bucket++) {
            if ((old_states[src_bucket].flag_probe & 1) == State::FILLED) {
...
private:
    ...
    size_t find_empty_bucket(const KeyT& key)
    {
        const auto main_bucket = _hasher(key) & _mask;
#if 0
        const auto bucket1 = bucket_from;
        if (!is_filled(bucket1))
            return bucket1;

        const auto bucket2 = bucket_from + 1;
        if (!is_filled(bucket2))
            return bucket2;
#endif
        ...
        auto probe = _states[main_bucket].get_probe();
        const auto next_bucket = (main_bucket + probe);
        const auto bmask = *(uint64_t*)(_states + (next_bucket & _mask)) & STAT_MASK;
        if (bmask != 0) {
            probe += CTZ(bmask) / STAT_BITS;
            _states[main_bucket].set_probe(probe + 1);
            return (main_bucket + probe) & _mask;
        }

        probe -= next_bucket % STAT_SKIP;

        while ( true ) {
            probe += STAT_SKIP;
            const auto bmask = *(uint64_t*)(_states + ((main_bucket + probe) & _mask)) & STAT_MASK;
            if (bmask != 0) {
                probe += CTZ(bmask) / STAT_BITS;
                _states[main_bucket].set_probe(probe + 1);
                return (main_bucket + probe) & _mask;
...
private:
    ...
    struct State
    {
        ...
        uint8_t flag_probe;
        static constexpr uint32_t over_probe = (1 << (sizeof(flag_probe) * 8 - 1)) - 1; // 127

        void inline set_probe(uint32_t probe)
        {
            if (probe < over_probe)
                flag_probe = (probe << 1) + (flag_probe & 1);
            else {
                if (probe > max_probe)
                    max_probe = probe;
                flag_probe = (over_probe << 1) + (flag_probe & 1);
            }
        }

        uint32_t inline get_probe() const
        {
            if (flag_probe >> 1 != over_probe)
                return flag_probe >> 1;
            return max_probe;
        }
    ...
    uint32_t max_probe        = State::over_probe;
};
